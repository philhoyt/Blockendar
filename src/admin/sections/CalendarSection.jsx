	...
	return (
		<VStack spacing={ 5 }>
			...
			<SelectControl
				...
				help={ __( 'The view shown when a visitor first loads the calendar block.', 'blockendar' ) }
			...
			<SelectControl
				...
				onChange={ ( val ) => update( { calendar_first_day: parseInt( val, 10 ) } ) }
				__nextHasNoMarginBottom
			/>

			<SelectControl
				label={ __( 'Time slot duration', 'blockendar' ) }
				help={ __( 'Height of each time slot in week/day views.', 'blockendar' ) }
				value={ settings.calendar_slot_duration ?? '00:30:00' }
				options={ SLOT_DURATION_OPTIONS }
				onChange={ ( val ) => update( { calendar_slot_duration: val } ) }
				__nextHasNoMarginBottom
			/>
		</VStack>
	);

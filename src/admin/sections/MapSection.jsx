...
const PROVIDER_OPTIONS = [
	{
		label: __(
			'OpenStreetMap / Leaflet (free, no key required)',
			'blockendar'
		),
		value: 'openstreetmap',
	},
	{
		label: __( 'Google Maps (requires API key)', 'blockendar' ),
		value: 'google',
	},
];
		...
		<VStack spacing={ 5 }>
			...
			{ provider === 'google' && (
				<TextControl
					...
					help={ __(
						'Required for Google Maps embed. Get a key from the Google Cloud Console.',
						'blockendar'
					) }
					value={ settings.google_maps_api_key ?? '' }
					type="password"
					onChange={ ( val ) =>
						update( { google_maps_api_key: val } )
					}
					__nextHasNoMarginBottom
				/>
			) }

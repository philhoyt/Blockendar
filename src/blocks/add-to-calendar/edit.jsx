...
import { PanelBody, TextControl, ToggleControl, __experimentalVStack as VStack } from '@wordpress/components';
import { __ } from '@wordpress/i18n';

export function Edit( { attributes, setAttributes } ) {
	const {
		label,
		showGoogle, showIcal, showOutlook365, showOutlookLive,
	} = attributes;

	const displayLabel = label || __( 'Add to Calendar', 'blockendar' );
	const blockProps   = useBlockProps( { className: 'blockendar-add-to-calendar' } );
				...
				<PanelBody title={ __( 'Settings', 'blockendar' ) }>
					<VStack spacing={ 3 }>
						<TextControl
							...
							placeholder={ __( 'Add to Calendar', 'blockendar' ) }
							__nextHasNoMarginBottom
						/>
						<ToggleControl
							label={ __( 'Google Calendar', 'blockendar' ) }
							checked={ showGoogle }
							onChange={ ( val ) => setAttributes( { showGoogle: val } ) }
							__nextHasNoMarginBottom
						/>
						<ToggleControl
							label={ __( 'iCalendar', 'blockendar' ) }
							checked={ showIcal }
							onChange={ ( val ) => setAttributes( { showIcal: val } ) }
							__nextHasNoMarginBottom
						/>
						<ToggleControl
							label={ __( 'Outlook 365', 'blockendar' ) }
							checked={ showOutlook365 }
							onChange={ ( val ) => setAttributes( { showOutlook365: val } ) }
							__nextHasNoMarginBottom
						/>
						<ToggleControl
							label={ __( 'Outlook Live', 'blockendar' ) }
							checked={ showOutlookLive }
							onChange={ ( val ) => setAttributes( { showOutlookLive: val } ) }
							__nextHasNoMarginBottom
						/>
					</VStack>
				</PanelBody>
			...
			<div { ...blockProps }>
				<div className="blockendar-add-to-calendar__dropdown">
					...
					<ul className="blockendar-add-to-calendar__menu" style={ { display: 'none' } }>
						...
						{ showOutlookLive && <li><span className="blockendar-add-to-calendar__item">{ __( 'Outlook Live',    'blockendar' ) }</span></li> }
					</ul>
				</div>
			</div>
		</>

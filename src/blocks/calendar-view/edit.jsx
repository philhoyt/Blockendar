...
function TermCheckboxList( { terms, selected, onChange, emptyLabel } ) {
	if ( ! terms.length ) {
		return (
			<p style={ { margin: 0, color: '#757575', fontSize: '12px' } }>
				{ emptyLabel }
			</p>
		);
	}
	...
	const toggle = ( id, checked ) => {
		if ( checked ) {
			...
			const base =
				selected.length === 0 ? terms.map( ( t ) => t.id ) : selected;
...
export function Edit( { attributes, setAttributes } ) {
	...
	const blockProps = useBlockProps( {
		className: 'blockendar-calendar-editor-preview',
	} );

...
document.querySelectorAll( '.blockendar-event-countdown' ).forEach( ( el ) => {
	...
	const tick = () => {
		...
		el.innerHTML =
			...
			`<span class="blockendar-countdown__segment"><strong>${ pad( hours ) }</strong> h</span>` +
			`<span class="blockendar-countdown__segment"><strong>${ pad( minutes ) }</strong> m</span>` +
			`<span class="blockendar-countdown__segment"><strong>${ pad( seconds ) }</strong> s</span>`;
